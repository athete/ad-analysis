...
class AXOHistFactory(processor.ProcessorABC):
    def __init__(self, triggers, hists_to_process):
        object_cuts = {
            'ScoutingPFJet': [('pt', 30.), ('eta', 3.0)],
            'ScoutingElectron': [('pt', 10.), ('eta', 2.65)],
            'ScoutingPhoton': [('pt', 10.), ('eta', 2.65)],
            'ScoutingMuonVtx': [('pt', 3.), ('eta', 2.4)],
        }
        ...
        self.trigger_axis = hist.axis.StrCategory([], name="trigger", label="Trigger", growth=True)
        self.object_axis = hist.axis.StrCategory([], name="object", label="Object", growth=True)
        ...
        self.met_axis = hist.axis.Regular(250, 0, 2500, name="met", label=r"$p^{\rm miss}_T$ [GeV]")
    ...
    def process(self, events):
        ...
        selection.add_multiple(
            {
                ...
                "saturatedL1MET": dak.all(
                    dak.flatten(events.L1EtSum.pt[(events.L1EtSum.etSumType==2) & (events.L1EtSum.bx == 0)]) < 1040, axis=1
                )
            }
        )
        ...
        if "pt" in self.hists_to_process:
            self.hists["pt"] = create_1d_hist(self.trigger_axis, self.pt_axis, self.object_axis)
        if "LeadingPt" in self.hists_to_process:
            self.hists["LeadingPt"] = create_1d_hist(self.trigger_axis, self.pt_axis, self.object_axis)
        if "SubleadingPt" in self.hists_to_process:
            self.hists["SubleadingPt"] = create_1d_hist(self.trigger_axis, self.pt_axis, self.object_axis)
        if "eta" in self.hists_to_process:
            self.hists["eta"] = create_1d_hist(self.trigger_axis, self.eta_axis, self.object_axis)
        if "phi" in self.hists_to_process:
            self.hists["phi"] = create_1d_hist(self.trigger_axis, self.phi_axis, self.object_axis)
